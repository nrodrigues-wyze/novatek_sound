...
def create_modules(module_defs, img_size):
    ...
    amin = 1e-10
    top_db = None
    window_size, hop_size, window_size, sample_rate, mel_bins, fmin, fmax = 1024, 320, 1024, 8000, 64, 20, 14000
    ...
    for i, mdef in enumerate(module_defs):
        ...
        if mdef['type'] == 'convolutional':
            ...
            modules.add_module('FC_Layer', ops.FullConnected(
                in_channels= 512, 
                out_channels = filters,
                quantize = is_quantize,
                activation_fn = None,
...
class MobileNetv2(nn.Module):
    ...
    def bn_forwardpass(self, x):
        input = x[0].transpose(1,3)
        clip_min_pre = 0.
        self.img_var = float(2**(x[1]) - 1)
        conv_bias = torch.zeros(64, device=x[0].device)
        prelu_alpha_qf = torch.ones(64, device=x[0].device)

        self.old_mean = self.b0.batch_norm.running_mean.clone(
            ).detach().requires_grad_(False)
        self.old_var = self.b0.batch_norm.running_var.clone(
            ).detach().requires_grad_(False)
        out, a, b = self.b0(input)
        ...
        if self.is_fixpoint and self.training:
            new_mean, new_var = self.b0.batch_norm.running_mean, self.BatchNorm2d.batch_norm.running_moving_mean = 10. * (new_mean - 0.9 * self.old_mean)
            moving_var = 10. * (new_var - 0.9 * self.old_var)
        else:
            moving_mean = self.b0.batch_norm.running_mean.clone().detach().requires_grad_(False)
            moving_var = self.b0.batch_norm.running_var.clone().detach().requires_grad_(False)
        wscale = torch.zeros(64, device=input.device)
        #self.img_var = float(2**(x[1]) - 1)
        conv_res = out
        #print("Here")
        out = ops.QuantizeFeature([out, conv_bias, wscale, alpha, beta, moving_mean, moving_var, prelu_alpha_qf, conv_res, self.img_var, clip_min_pre])
        return out
    ...
    def forward(self, x, verbose=False):
        ...
        x = (torch.clamp(x,-100,28)+100.0)/128.0
        ...
        for i, (mdef, module) in enumerate(zip(self.module_defs, self.module_list)):
            ...
            if mtype in ['convolutional', 'unpooling', 'maxpool','conv_dw']:
                ...
                return sound_out
                #return {'clipwise_output': clipwise_output, 'embedding': None}
